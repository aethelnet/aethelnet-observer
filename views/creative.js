/**
 * Creative View Module
 * 3D market connections visualization
 */

async function init(container, params = {}) {
    console.log('[Creative] Initializing...', params);
    
    try {
        const response = await fetch('./creative/index.html');
        ...
        console.log('[Creative] Initialized');
    } catch (error) {
        console.error('[Creative] Failed to load:', error);
        container.innerHTML = `<div style="padding: 40px; text-align: center; color: #f87171;"><h2>Error Loading Creative</h2><p>${error.message}</p></div>`;
    }
}

async function destroy() {
    console.log('[Creative] Destroying...');
    if (window.creativeUpdateInterval) {
        clearInterval(window.creativeUpdateInterval);
        window.creativeUpdateInterval = null;
    }
    console.log('[Creative] Destroyed');
}
