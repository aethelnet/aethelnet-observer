...
async function init(container, params = {}) {
    console.log('[Analytics] Initializing...', params);
    
    try {
        const response = await fetch('./analytics/index.html');
        ...
        container.innerHTML = `<div style="padding: 40px; text-align: center; color: #f87171;"><h2>Error Loading Analytics</h2><p>${error.message}</p></div>`;
    }
}

async function destroy() {
    console.log('[Analytics] Destroying...');
    if (window.analyticsUpdateInterval) {
        clearInterval(window.analyticsUpdateInterval);
        window.analyticsUpdateInterval = null;
    }
