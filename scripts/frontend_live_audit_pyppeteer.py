#!/usr/bin/env python3
"""
frontend_live_audit_pyppeteer.py

Lightweight frontend live verification using pyppeteer (headless Chromium).

What it does:
- Launches a headless Chromium instance using pyppeteer.
...
- If Chromium can't launch due to sandboxing in some environments, pyppeteer launch args include --no-sandbox.
"""
...
try:
    ...
    print("ERROR: pyppeteer is required. Install it with: pip install pyppeteer", file=sys.stderr)
    print("If installation fails, try: pip install --upgrade pyppeteer", file=sys.stderr)
    sys.exit(1)
except Exception as e:
    print(f"ERROR: Failed to import pyppeteer: {e}", file=sys.stderr)
    print("Try reinstalling: pip install --upgrade pyppeteer", file=sys.stderr)
    sys.exit(1)

# Configuration
HOST = "localhost"
PORTS = list(range(1420, 1427))  # 1420..1426 inclusive
...
OUTPUT_JSON = Path("frontend_live_audit_pyppeteer_report.json")
...
async def probe_port(browser, port: int) -> Dict[str, Any]:
...
async def main() -> int:
    ...
    try:
        browser = await launch(
            {"headless": True, "args": ["--no-sandbox", "--disable-setuid-sandbox"], "defaultViewport": None}
        )
    except Exception as e:
        print(f"ERROR: Failed to launch browser: {e}", file=sys.stderr)
        print("This may be due to missing Chromium. Pyppeteer should auto-download it.", file=sys.stderr)
        print("If the issue persists, try:", file=sys.stderr)
        print("  - Check internet connection (for Chromium download)", file=sys.stderr)
        print("  - Try: pip install --upgrade pyppeteer", file=sys.stderr)
        return 1

    sem = asyncio.Semaphore(CONCURRENT_PAGES)

    async def _probe_with_sem(p):
        async with sem:
            return await probe_port(browser, p)

    tasks = [_probe_with_sem(p) for p in PORTS]
